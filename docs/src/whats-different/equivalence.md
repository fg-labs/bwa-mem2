...
**Where the upstreams disagree, each target follows its own.** On a record where bwa and
bwa-mem2 emit different *alignments*, `--compat=bwa-mem` matches bwa and `--compat=bwa-mem2`
matches bwa-mem2 — the table records per-target behavior, which is why both targets can be
right at once. Two such records are known (below); neither is reachable on the cells measured
here, and the rare-event tail is not proven empty — see the statistical-power caveat below.

That is the reason both targets exist as a table rather than a boolean, and the shape is not
hypothetical: bwa-mem2's own tracker carries reports of exactly this kind. The
best-documented one,
...
`--compat=<target>` reproduces its target's output, records and header alike (`@PG` excluded). Almost all of
that is output shaping — the targets differ from each other and from the default only in
tags and header — except on the records where the two upstreams themselves emit different
alignments. bwa-mem2 advertises output identical to bwa, and two such records are known,
both places where its port is not faithful. On each, `--compat=bwa-mem` reproduces bwa,
`--compat=bwa-mem2` reproduces bwa-mem2, and the default path takes bwa's answer, the
principled one (without `--compat`, bwa-mem3 is bwa-mem2 plus bug fixes). First, when the
chain weight filter drops every chain for a read, bwa leaves it unmapped and bwa-mem2 aligns
it from the chain it just rejected ([#310](https://github.com/fg-labs/bwa-mem3/issues/310);
default fixed in [#489](https://github.com/fg-labs/bwa-mem3/pull/489)). The path is
unreachable without `-W` or an `-x pacbio`/`pbref`/`ont2d` preset, since `min_chain_weight`
defaults to 0. Second, the suffix-array sentinel offset
...
- **`--compat` and `--fast` are mutually exclusive** — passing both is a hard error.
  `--compat` asks for parity with a pinned upstream; `--fast` deliberately moves alignments
  away from both, so `--fast --compat=bwa-mem2` would produce a diff-clean-looking stream over
  genuinely different alignments. `--compat` is meaningful only on the drop-in profile.
...
- **Score-gated chain-extension cap and its `MAPQ` seed ([#269](https://github.com/fg-labs/bwa-mem3/pull/269), default on under `--fast`).** Two coupled opt-in levers. `--extend-tie-frac` (with `--extend-tie-floor`) extends a chain to banded-SW only when its weight is within a fraction of the best chain's, so a non-competitive tail chain that the default would have scored is **removed before extension** — which can drop a candidate secondary and move `XS`/secondary/`MAPQ`. `--extend-csub` then feeds the best pruned chain's weight back in as a competitor-score estimate so the pruning does not *inflate* `MAPQ`; this changes emitted `MAPQ` on **both** the scalar single-end path (`mem_approx_mapq_se`) and the batched paired-end pairing path (`mem_capped_pair_subo`). All three options (`--extend-tie-frac`, `--extend-tie-floor`, `--extend-csub`) are off by default, and with them off the gate is a no-op: the alignment records are md5-identical to the baseline build — verified over a 197k-pair panel slice (404,199 records) on both the default path and `--fast` with the gate off, `@PG` and the additive `MQ:i`/`HN:i` tags excluded as elsewhere on this page. `--fast` folds them in — in both non-meth and `--meth` mode — at `--extend-tie-frac 0.95 --extend-tie-floor 1` with `--extend-csub` forced on, where the combination is **not** byte-identical by design. On truth-graded simulations (WGS, exome, panel-targets, and methylation) the gate is placement-neutral — worst-case −0.007 pp correct-placement delta vs the uncapped baseline; the movement is `MAPQ` recalibration (`csub`) and low-confidence secondaries, with the `MAPQ`-error-rate measurements taken on an AWS c6a.4xlarge host (AMD EPYC Milan, x86_64, AVX2 tier). Read those figures as scoped to those workloads and that host/tier, not a guarantee across all inputs and architectures. See [Features → `--extend-tie-frac`](features.md) and [modes → `--fast`](modes.md).
- **Signed-`char` nt4-encoding hardening (`bntseq.cpp`, `bwamem.cpp`; [#456](https://github.com/fg-labs/bwa-mem3/pull/456)).** The nt4 re-encoding of sequence bytes indexed `nst_nt4_table` / compared against the encoded range using a plain `char`. On signed-`char` platforms (x86) a byte `>= 0x80` is negative, so it passed the `< 4`/`< 5` guard and was stored verbatim (and `add1` indexed the 256-entry table at a negative offset — an out-of-bounds read), feeding an out-of-range value to the scoring matrix. Casting the guard operand to `unsigned char` routes such a byte through the table to `N` (code 4). This affects **only** reads or references containing bytes `>= 0x80` (i.e. non-ACGTN, corrupt/adversarial input); all ACGTN and pre-encoded input is unchanged. On unsigned-`char` platforms (ARM) `char` was already unsigned and no byte reached the buggy branch, so there is no change there — the fix makes x86 match the behavior ARM already had, **removing** a per-architecture divergence rather than adding one. Equivalence on valid input here is **by construction, not a measured concordance claim**: the stored value differs from the pre-fix code *only* for input bytes `>= 0x80`, which ACGTN and pre-encoded input never contain, so every real read is unchanged independent of workload, host, architecture, or SIMD tier — there is nothing per-fixture to attribute. This entry therefore asserts **no** SAM byte-identity result, and in particular says nothing about the header block; it is a statement about the *alignment-record* bytes for the reads the re-encoding touches. The signed-`char` bug could fire only on x86; ARM was already correct, so no fixture exercises a change on it.
- **All chains dropped by the weight filter ([#310](https://github.com/fg-labs/bwa-mem3/issues/310), default fixed in [#489](https://github.com/fg-labs/bwa-mem3/pull/489)).** When `-W` or a long-read preset makes the weight filter drop every chain for a read, bwa-mem2 aligns the read from the chain it just rejected, through a count its seqid-range bookkeeping rebuilds for the emptied array; bwa leaves the read unmapped. The default now follows bwa; `--compat=bwa-mem2` reproduces bwa-mem2. Unreachable at default settings, since `min_chain_weight` defaults to 0.
...
- **Banded-SW `qlen` band-clamp overflow fix ([#468](https://github.com/fg-labs/bwa-mem3/pull/468)).** The batch SW wrappers (`smithWatermanBatchWrapper{8,16}`) pre-scaled the per-lane clamp reach as `len2 * max_sc` into a narrow `uint8_t` (8-bit tiers) / `uint16_t` (16-bit tiers) slot, which wraps once the product exceeds 255 / 65535. `max_sc` is the largest scoring-matrix entry, which for any standard matrix is the match reward `A` — the mismatch and ambiguous entries are stored negative — so the overflow trigger is `len2 * A` crossing the slot ceiling and does **not** depend on `-B`. A wrapped reach makes the vector band **narrower** than `scalarBandedSWA` computes (which uses native `int`), so the extension can miss an off-diagonal optimum the scalar reference finds. The fix carries the reach in a wide `int32_t` slot, matching the scalar band formula exactly; bwa-mem2 inherits the same narrow-slot overflow, so this diverges from bwa-mem2 on the affected reads while converging bwa-mem3's vector tiers onto its own scalar reference. **The reachable case is the 16-bit tier only:** on the 8-bit tier the production routing envelope (`bsw8_envelope_ok`) admits a pair only when `len2 * A` is already below 255, so the narrow `uint8_t` slot never overflows on a routed pair and the 8-bit widening is **defensive** (no shipped-output change); the divergence is confined to the 16-bit tier with a long query segment at non-default `-A ≥ 3` (`len2 * A > 65535` — e.g. `len2 ≳ 21846 bp` at `-A3`) carrying an off-diagonal indel. (`-A2` cannot reach it: the ref-window length guard caps `len2 ≤ 32767`, so `len2 * 2 ≤ 65534` never overflows the `uint16_t` slot.) Latent on near-diagonal input regardless of tier (a wider band cannot change a near-diagonal optimum), so on normal read lengths the fix is byte-identical. A full-field kernel byte-identity gate — every result field per pair for both the extension (`score`/`tle`/`gtle`/`qle`/`gscore`/`max_off`) and mate-rescue (`score`/`te`/`qe`/`score2`/`te2`/`tb`/`qb`) kernels, 60,009 pairs per cell — recorded **zero** discordant pairs at query/reference lengths 100/150/250: at bwa's default `-A1` (and a 10%-ambiguous-base variant) and at non-default `-A3` on the forced 16-bit tier, with the batch composition held fixed across the A/B (fixed `-K`, so both builds route identical lanes); measured on Apple Silicon (M2 Max, arm64, NEON tier). The whole-aligner byte-identity harness is likewise unchanged on chr22 / WGS / `--meth`, and the x86 SSE4.1 / AVX2 / AVX-512BW tiers are built and exercised on every CI run. Every result here is scoped to fixed batching. It moves only reads whose optimum needs a band wider than the wrapped value — reachable only in the 16-bit long-query `-A ≥ 3` case above, which the `test/unit/test_bandedswa_band_clamp.cpp` regression covers directly (it fails on the narrow-slot kernel and passes on the wide-slot fix).
- **Banded-SW 16-bit z-drop gap-extend weighting ([#471](https://github.com/fg-labs/bwa-mem3/pull/471)).** `scalarBandedSWA` gates its z-drop early-exit on `(max - m) - |drift| * e_{del|ins} > zdrop`, weighting the off-diagonal drift by the gap-extend penalty (the `* e_del` / `* e_ins` factors in its `else if (zdrop > 0)` block). The three 16-bit vector kernels (`smithWaterman{256,512,128}_16`, via the `ZSCORE16` macro) computed the direction-appropriate `insdel` penalty but dropped the multiply, testing `(max - m) - |drift|` instead. The 8-bit kernels and the scalar reference both apply the factor (the 8-bit kernels via a wide-int32 z-drop), so only the 16-bit tiers were affected. At the default `-E 1` the factor is 1, so this is a **no-op there and byte-identical**; it diverges only at non-default gap-extend (`-E > 1`) on reads whose optimal alignment goes off-diagonal (an indel) and drops in score near the z-drop horizon, where the under-weighted term z-dropped the vector extension earlier than the scalar — bwa-mem2's 16-bit kernel has the same omission, so the fix diverges from bwa-mem2 on those reads while converging bwa-mem3's 16-bit tier onto its own scalar reference. The fix forms `|drift| * e` in wide `int32` and narrows with a saturating pack, so the product cannot wrap `int16` (on saturation the residual drop is deeply negative and the z-drop cannot fire, which the scalar's native-int arithmetic also yields). Latent on the default-`-E` byte-identity harness: the isolated SW-kernel byte-identity gate compares every result field per aligned pair and recorded **zero discordant pairs** on the NEON 8-bit and 16-bit kernels for both the extension and mate-rescue paths — 60,009 pairs per mode at query/reference lengths 100/150/250, 10% ambiguous bases, and a pair count not divisible by the 16-lane width (so padded lanes are covered) — and the whole-aligner byte-identity check passes on chr22 `-t1`/`-t8` and `--meth`; measured on Apple Silicon (M2 Max, arm64, NEON tier). Default-`-E` extend throughput on the affected 16-bit tier is neutral (isolated-kernel A/B: +1.3% at length 250, inside the run-to-run noise floor bounded by the byte-identical 8-bit tier's own ±0.9–3.3% swing across the same A/B — no regression). The non-default-`-E` correctness is checked by a fails-before/passes-after differential unit test against `scalarBandedSWA` (`test/unit/test_bandedswa_zdrop_gate.cpp`), which runs on every CI SIMD tier (NEON, SSE4.1, AVX2, AVX-512BW), so each tier's kernel is exercised at `-E > 1` against the scalar oracle.
- **Banded-SW 16-bit x86 gap-open unsigned-subtract clamp ([#480](https://github.com/fg-labs/bwa-mem3/pull/480)).** The AVX2 (256-bit) and AVX-512BW (512-bit) 16-bit banded-extension cores (`MAIN_CODE16_CORE`) formed the gap-open term with `subs_epu16(m11, open)` — unsigned-saturating subtraction, correct only while `m11 ≥ 0`. But `m11 = h00 + sbt11` goes **negative** when a positive diagonal `H` meets a mismatch or ambiguous-base (`N`) penalty, and unsigned-saturating subtract then reads that negative value as a large unsigned, so the term stays negative and leaks a sub-zero `E`/`F` into the band's leading/trailing zero-scan. A stray `-1` there shifts `head`/`tail` and corrupts **`gtle`** (the target end coordinate) on a degenerate alignment where every chain is filtered below `h0`. The 128-bit core already used a signed `sub` + `max(., 0)` (an explicit zero floor) and was correct; the fix clamps `m11 ≥ 0` once up front and keeps the cheap `subs_epu16`, matching the scalar oracle's signed-sub-then-floor for one added op. bwa-mem2 inherits the same narrow unsigned-subtract in its 16-bit cores, so the fix **diverges from bwa-mem2** on the affected reads while converging bwa-mem3's wide x86 tiers onto its own scalar reference and 128-bit core. **The reachable case is the x86 16-bit tier only** (the 128-bit/`sse41` and NEON cores were already correct), confined to the degenerate all-chains-filtered case the parity harness surfaces (1 pair in 512 at the harness's input); it is latent — byte-identical — on all non-degenerate input. A kernel-level cross-tier byte-identity gate — emit the golden on the correct 128-bit (`sse41`) tier and check it on the fixed 256-bit (`avx2`) tier — recorded the fixed AVX2 path **byte-identical to the 128-bit path across 60,009 extension pairs** (query/reference lengths 100/150/250, 10% ambiguous bases, and a pair count not divisible by the 16-lane width so padded lanes are covered) on **every** result field, with the pair batch held fixed and identical across the two tiers — the same 60,009 pairs in the same order, so both tiers pack the same lanes and the same padding (the isolated-kernel analog of pinning `-K`, since no aligner batching is involved in a kernel-only gate); measured on an AWS c6a.4xlarge host (AMD EPYC Milan / Zen3, x86_64, AVX2 tier), clang-19, with the unit suite passing (287/287) on that tier. The AVX-512BW 512-bit core carries the **identical source fix** but was not separately hardware byte-identity-validated (the repro host was AVX2-only); it is built and exercised against the scalar oracle on every CI AVX-512BW run by the differential parity case this PR adds (`test/unit/test_bandedswa_asym.cpp`, an all-fields + ambiguous-`N` comparison against `scalarBandedSWA`, with a tier-exposure sentinel so the cross-tier coverage is explicit). This entry asserts a **kernel-level** byte-identity result and a per-architecture correctness convergence; it makes **no** whole-aligner x86 SAM byte-identity claim for this fix, and says nothing about the header block. Throughput is a **separate** isolated-kernel measurement, kept distinct from the byte-identity gate above: run in isolation, the 16-bit extension kernel costs about **−4 to −5%** from the added clamp op — 250 bp query −5.2%, 1000 bp query −4.3% vs the blessed base — measured with a standalone single-kernel throughput benchmark on the same AWS c6a.4xlarge host (AMD EPYC Milan / Zen3, x86_64, AVX2 tier), clang-19. Being a single-kernel figure, it has no aligner thread count or batch size to report; the 8-bit path and short-read alignment are untouched.
